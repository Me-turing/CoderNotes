<template><div><p>由于多线程的特性，由于多个线程互相抢占资源，可能会出现资源不可控的现象：</p>
<p>比如：抢票软件出现了 重票，错票。主要就是：多个线程，在争抢资源的过程中，导致共享的资源出现问题。一个线程还没执行完，另一个线程就参与进来了，开始争抢。</p>
<p>为此，我们需要对线程进行同步操作，也就是加锁。</p>
<h2 id="同步代码块" tabindex="-1"><a class="header-anchor" href="#同步代码块" aria-hidden="true">#</a> 同步代码块</h2>
<h3 id="语法" tabindex="-1"><a class="header-anchor" href="#语法" aria-hidden="true">#</a> 语法</h3>
<p><code v-pre>synchronized (同步监视器){}</code></p>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">BuyTicketThread</span> <span class="token keyword">implements</span> <span class="token class-name">Runnable</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="关于同步监视器" tabindex="-1"><a class="header-anchor" href="#关于同步监视器" aria-hidden="true">#</a> 关于同步监视器</h3>
<ol>
<li>必须是引用数据类型，不能是基本数据类型</li>
<li>也可以创建一个专门的同步监视器，没有任何业务含义</li>
<li>一般使用共享资源做同步监视器即可</li>
<li>在同步代码块中不能改变同步监视器对象的引用</li>
<li>尽量不要String和包装类Integer做同步监视器</li>
<li>建议使用final修饰同步监视器</li>
<li>也可以利用反射的原理，使用 <code v-pre>类名.class </code>字节码作为同步监视器</li>
</ol>
<figure><img src="@source/CoderNotes/01_JavaSE阶段/B_进阶/05_多线程/05_线程安全/image/image_oImT5VO39O.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<h4 id="执行过程" tabindex="-1"><a class="header-anchor" href="#执行过程" aria-hidden="true">#</a> 执行过程</h4>
<ol>
<li>第一个线程来到同步代码块，发现同步监视器open状态，需要close,然后执行其中的代码</li>
<li>第一个线程执行过程中，发生了线程切换（阻塞 就绪），第一个线程失去了cpu，但是没有开锁open</li>
<li>第二个线程获取了cpu，来到了同步代码块，发现同步监视器close状态，无法执行其中的代码，第二个线程也进入阻塞状态</li>
<li>第一个线程再次获取CPU,接着执行后续的代码；同步代码块执行完毕，释放锁open</li>
<li>第二个线程也再次获取cpu，来到了同步代码块，发现同步监视器open状态，拿到锁并且上锁，由阻塞状态进入就绪状态，再进入运行状态，重复第一个线程的处理过程（加锁）</li>
<li>建议使用final修饰同步监视器</li>
</ol>
<h4 id="注意" tabindex="-1"><a class="header-anchor" href="#注意" aria-hidden="true">#</a> 注意</h4>
<ol>
<li>同步代码块中能发生CPU的切换吗？能！！！ 但是后续的被执行的线程也无法执行同步代码块（因为锁仍旧close）</li>
...
<p><code v-pre>public static`` ``synchronized`` ``void buyTicket(){}</code></p>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">BuyTicketThread</span> <span class="token keyword">extends</span> <span class="token class-name">Thread</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="注意-1" tabindex="-1"><a class="header-anchor" href="#注意-1" aria-hidden="true">#</a> 注意</h4>
<ol>
<li>不要将<code v-pre>run()</code>定义为同步方法</li>
<li>非静态同步方法的同步监视器是this，静态同步方法的同步监视器是 <code v-pre>类名.class</code> 字节码信息对象</li>
<li>同步代码块的效率要高于同步方法，因为：同步方法是将线程挡在了方法的外部，而同步代码块锁将线程挡在了代码块的外部，但是却是方法的内部</li>
<li>同步方法的锁是this，一旦锁住一个方法，就锁住了所有的同步方法；同步代码块只是锁住使用该同步监视器的代码块，而没有锁住使用其他监视器的代码块</li>
</ol>
<h3 id="lock锁" tabindex="-1"><a class="header-anchor" href="#lock锁" aria-hidden="true">#</a> Lock锁</h3>
<p>JDK1.5后新增新一代的线程同步方式 :  <strong>Lock锁，与采用synchronized相比，lock可提供多种锁方案，更灵活</strong></p>
<p><code v-pre>synchronized</code>是Java中的关键字，这个关键字的识别是靠JVM来识别完成的，<strong>是虚拟机级别</strong>的。但是<strong>Lock锁是API级别</strong>的，提供了相应的接口和对应的实现类，这个方式更灵活，表现出来的性能优于之前的方式</p>
<h4 id="相关概念" tabindex="-1"><a class="header-anchor" href="#相关概念" aria-hidden="true">#</a> 相关概念</h4>
<h5 id="condition等待池" tabindex="-1"><a class="header-anchor" href="#condition等待池" aria-hidden="true">#</a> Condition等待池</h5>
<p>Condition是在Java 1.5中才出现的，它用来替代传统的Object的<code v-pre>wait()</code>、<code v-pre>notify()</code> 实现线程间的协作，相比使用Object的<code v-pre>wait()</code>、<code v-pre>notify()</code>，使用Condition的<code v-pre>await()</code>、<code v-pre>signal()</code>这种方式实现线程间协作更加安全和高效。</p>
<p>它能够更加精细的控制多线程的休眠与唤醒。对于同一个锁，我们可以创建多个Condition，在不同的情况下使用不同的Condition</p>
<p>一个<code v-pre>Condition</code>包含一个等待队列。一个<code v-pre>Lock</code>可以产生多个<code v-pre>Condition</code>，所以可以有多个等待队列。</p>
<p>在<code v-pre>Object</code>的监视器模型上，一个对象拥有一个同步队列和等待队列，而<code v-pre>Lock</code>（同步器）拥有一个同步队列和多个等待队列。​</p>
<p>Object中的<code v-pre>wait()</code>,<code v-pre>notify()</code>,<code v-pre>notifyAll()</code>方法是和&quot;同步锁&quot;(<code v-pre>synchronized关键字</code>)捆绑使用的；而<code v-pre>Condition</code>是需要与&quot;<code v-pre>互斥锁</code>&quot; / &quot;<code v-pre>共享锁</code>&quot; 捆绑使用的。</p>
<p>调用Condition的<code v-pre>await()</code>、<code v-pre>signal()</code>、<code v-pre>signalAll()</code>方法，都必须在lock保护之内，就是说必须在<code v-pre>lock.lock()</code>和<code v-pre>lock.unlock</code>之间才可以使用</p>
<ul>
...
<blockquote>
<p>注意 :<br>
<code v-pre>await()</code>的线程在接到信号或被中断之前一直处于等待状态：<br>
与此 Condition 相关的锁以原子方式释放，并且出于线程调度的目的，将禁用当前线程，且在发生以下四种情况之一 以前，当前线程将一直处于休眠状态：</p>
<ol>
<li>其他某个线程调用此 Condition 的 <code v-pre>signal() 方法</code>，并且碰巧将当前线程选为被唤醒的线程；</li>
<li>其他某个线程调用此 Condition 的 <code v-pre>signalAll() 方法</code>；</li>
<li>其他某个线程<code v-pre>中断当前线程</code>，且支持<code v-pre>中断线程</code>的挂起；</li>
<li>发生“<code v-pre>虚假唤醒</code>”</li>
</ol>
</blockquote>
<h4 id="语法-2" tabindex="-1"><a class="header-anchor" href="#语法-2" aria-hidden="true">#</a> 语法</h4>
<p><code v-pre>Lock lock = new ReentrantLock();</code></p>
<h4 id="api" tabindex="-1"><a class="header-anchor" href="#api" aria-hidden="true">#</a> API</h4>
<h5 id="lock锁的api" tabindex="-1"><a class="header-anchor" href="#lock锁的api" aria-hidden="true">#</a> Lock锁的API</h5>
<table>
<thead>
<tr>
<th>API</th>
...
<tbody>
<tr>
<td><code v-pre>ReentrantLock()</code></td>
<td>构造方法，默认不开启公平策略</td>
</tr>
<tr>
<td><code v-pre>ReentrantLock(boolean fair)</code></td>
<td>构造方法，是否开启公平策略，true 开始 false 不开启</td>
</tr>
<tr>
<td><code v-pre>void lock()</code></td>
<td>获得锁</td>
</tr>
<tr>
<td><code v-pre>void unlock()</code></td>
<td>释放此锁</td>
</tr>
<tr>
<td><code v-pre>Condition newCondition()</code></td>
<td>创建等待池</td>
</tr>
<tr>
<td><code v-pre>final int getQueueLength()</code></td>
<td>返回等待获取此锁的线程数的估计值</td>
</tr>
<tr>
<td><code v-pre>boolean hasWaiters(Condition condition)</code></td>
<td>查询是否有线程正在等待与此锁</td>
</tr>
<tr>
<td><code v-pre>int getWaitQueueLength(Condition condition)</code></td>
<td>返回在与此锁相关的给定条件上等待的线程数的估计值</td>
</tr>
</tbody>
</table>
<h5 id="condition-等待池的api" tabindex="-1"><a class="header-anchor" href="#condition-等待池的api" aria-hidden="true">#</a> Condition 等待池的API</h5>
<table>
<thead>
<tr>
<th>API</th>
...
<tbody>
<tr>
<td><code v-pre>void await()</code></td>
<td>当前线程放入等待池，直到收到信号或被中断</td>
</tr>
<tr>
<td><code v-pre>boolean await(long time, TimeUnit unit)</code></td>
<td>导致当前线程等待，直到发出信号或中断，或指定的等待时间结束。
Time→等待的最大时间单位  unit → 参数的时间单位</td>
</tr>
<tr>
<td><code v-pre>boolean awaitUntil(Date deadline)</code></td>
<td>导致当前线程等待，直到收到信号或中断，或超过指定的截止日期。
deadline → 等待的绝对时间</td>
</tr>
<tr>
<td><code v-pre>void signal()</code></td>
<td>唤醒一个等待的线程</td>
</tr>
<tr>
<td><code v-pre>void signalAll()</code></td>
<td>唤醒所有等待的线程</td>
</tr>
</tbody>
</table>
